...
■ 更新履歴
・Ver 2.1.3 -> Ver 2.1.4
	・モニタ毎のDPI(Per-Monitor V2)に対応した。
	　→拡大率の違うモニタ間でビューアを移動した場合やモニタの表示スケール
	　　を変更した場合に、サインアウトせずに表示が追従するようになった。
	・メニューはメニューを表示するモニタの拡大率で表示するようにした。
	・タスクトレイ、メニュー、ビューアのアイコンを拡大率に合った
	　サイズで表示するようにした。
	・英語版の設定画面で文字が欠けたり項目が重なる問題を修正した。
	　→ダイアログをDS_SHELLFONTに変更し、OSのシェルダイアログフォントで
	　　表示するようにした。(日本語版、英語版とも)
	　→ラベルやボタンの幅が足りず文字が欠ける箇所を調整した。
	・バイナリ表示とテキスト表示、ファイル表示がDPIの変更に追従しない
	　問題を修正した。
